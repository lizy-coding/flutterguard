...
class StateDependencyCycleRule {
  ...
  const StateDependencyCycleRule(this.config, {required this.projectPath});
  ...
  List<StaticIssue> analyze(
    ...
    for (final item in units) {
      ...
      for (final declaration
          in item.source.unit.declarations
              .whereType<TopLevelVariableDeclaration>()) {
        for (final variable in declaration.variables.variables) {
          final initializer = variable.initializer;
          final ownerId = _nodeId(item.file, 'provider', variable.name.lexeme);
    ...
    for (final component in _stronglyConnectedComponents(graph)) {
      ...
      final changedNodes =
          component.where((id) => targetSet.contains(nodes[id]!.file)).toList()
            ..sort();
  ...
  static String? _resolveNode({
    ...
    final local = candidates
        .where((id) => nodes[id]!.file == normalizedSource)
        .toList();
  ...
  String _displayName(_StateGraphNode node, Map<String, List<String>> names) {
    ...
    void connect(String node) {
      ...
      for (final next in neighbors) {
        if (!indices.containsKey(next)) {
          connect(next);
          lowlink[node] = lowlink[node]! < lowlink[next]!
              ? lowlink[node]!
              : lowlink[next]!;
        } else if (onStack.contains(next)) {
          lowlink[node] = lowlink[node]! < indices[next]!
              ? lowlink[node]!
              : indices[next]!;
        }
      }
