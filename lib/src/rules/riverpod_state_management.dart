...
class RiverpodReadUsedForRenderRule {
  ...
  const RiverpodReadUsedForRenderRule(this.config);
  ...
  List<StaticIssue> analyze(List<String> files, {SourceWorkspace? workspace}) {
    ...
    for (final file in files) {
      ...
      if (source == null ||
          !frameworkAllowed(source.unit, StateManagementFramework.riverpod)) {
        continue;
      }
    ...
    for (final read in visitor.reads) {
      ...
      if (variable != null) {
        ...
        for (final usage in usages.renderUsages) {
          sinks.add(
            'render use of $variable: ${compactEvidence(_nearestRenderNode(usage, root))}',
          );
        }
      }
...
class RiverpodWatchInCallbackRule {
  ...
  const RiverpodWatchInCallbackRule(this.config);
  ...
  List<StaticIssue> analyze(List<String> files, {SourceWorkspace? workspace}) {
    ...
    for (final file in files) {
      ...
      for (final callback in visitor.callbacks) {
        final watches = callback.watches;
