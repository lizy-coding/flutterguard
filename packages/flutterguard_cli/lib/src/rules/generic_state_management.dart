  ...
  List<StaticIssue> analyze(List<String> files, {SourceWorkspace? workspace}) {
    ...
    for (final file in files) {
      ...
      for (final root in buildRoots(source.unit)) {
        final visitor = _SideEffectVisitor(config.allowlist);
...
class _SideEffectVisitor extends RecursiveAstVisitor<void> {
  ...
  _SideEffectVisitor(this.allowlist);
  ...
  void visitInstanceCreationExpression(InstanceCreationExpression node) {
    ...
    if (type == 'Timer' && !allowlist.contains(type)) {
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    final notifierCommand = target.contains('ref.read') &&
...
class StateManagerCreatedInBuildRule {
  ...
  const StateManagerCreatedInBuildRule(
    this.config,
    this.stateManagement, {
    required this.projectPath,
  });
  ...
  List<StaticIssue> analyze(List<String> files, {SourceWorkspace? workspace}) {
    ...
    for (final file in files) {
      ...
      if (source == null) continue;
      for (final root in buildRoots(source.unit)) {
        final visitor = _StateManagerCreationVisitor(config.allowlist);
        root.body.accept(visitor);
...
class _StateManagerCreationVisitor extends RecursiveAstVisitor<void> {
  ...
  _StateManagerCreationVisitor(this.allowlist);
  ...
  void _record(AstNode node, String type) {
    final owned = stateOwnerSuffixes.any(type.endsWith) ||
        flutterControllers.contains(type);
    if (owned && !allowlist.contains(type)) {
  ...
  List<StaticIssue> _inspectClass(
    ...
    for (final field in cls.members.whereType<FieldDeclaration>()) {
      ...
      for (final variable in field.fields.variables) {
        ...
        final allowed = config.allowlist.contains('${cls.name.lexeme}.$name');
        if (allowed) continue;
...
class StateLayerUiDependencyRule {
  ...
  const StateLayerUiDependencyRule(
    this.config,
    this.stateManagement, {
    required this.projectPath,
  });
  ...
  List<StaticIssue> analyze(List<String> files, {SourceWorkspace? workspace}) {
    ...
    for (final file in files) {
      ...
      for (final cls
          in source.unit.declarations.whereType<ClassDeclaration>()) {
        if (!isStateOwnerClass(cls) ||
            config.allowlist.contains(cls.name.lexeme)) {
          continue;
        }
  ...
  static RuleMeta describe() => const RuleMeta(
        ...
        priority: 'p0',
        purpose:
            '检测状态 owner 对 BuildContext、Widget、Navigator、Theme 等 UI API 的依赖',
...
class _UiDependencyVisitor extends RecursiveAstVisitor<void> {
  ...
  _UiDependencyVisitor(this.className, this.allowlist);
  ...
  void visitNamedType(NamedType node) {
    ...
    if (!nestedTypeOnly &&
        uiTypes.contains(simple) &&
        !allowlist.contains('$className.$simple')) {
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    if ((uiTarget && uiCalls.contains(name)) ||
      ...
      if (!allowlist.contains('$className.$name')) {
        evidence.add(compactEvidence(node));
      }
    }
    super.visitMethodInvocation(node);
