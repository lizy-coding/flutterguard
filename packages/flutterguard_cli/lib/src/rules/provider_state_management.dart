...
class ProviderValueLifecycleMisuseRule {
  ...
  const ProviderValueLifecycleMisuseRule(
    this.config,
  ...
  List<StaticIssue> analyze(List<String> files, {SourceWorkspace? workspace}) {
    ...
    for (final file in files) {
      ...
      if (source == null ||
          !frameworkAllowed(
            source.unit,
            StateManagementFramework.provider,
            stateManagement,
          )) {
        continue;
      }
      final visitor = _ProviderOwnershipVisitor(config.allowlist);
...
class _ProviderOwnershipVisitor extends RecursiveAstVisitor<void> {
  ...
  _ProviderOwnershipVisitor(this.allowlist);
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    final providerType =
        targetIsProvider ? targetType! : simpleTypeName(node.methodName.name);
  ...
  void _inspectCall({
    ...
    if (constructor == 'value') {
      ...
      if (createdType != null && !isConst && !_isImmutableType(createdType)) {
        findings.add(_ProviderOwnershipFinding(
          node,
          'value_creates',
    ...
    final create = _argument(arguments, 'create') ??
  ...
  bool _isImmutableType(String type) {
    if (allowlist.contains(type)) return true;
...
class NotifyListenersInLoopRule {
  ...
  const NotifyListenersInLoopRule(
    this.config,
    this.stateManagement, {
    required this.projectPath,
  });
  ...
  List<StaticIssue> analyze(List<String> files, {SourceWorkspace? workspace}) {
    if (!stateRuleEnabled(config, stateManagement)) return [];
    if (config.allowlist.contains('notifyListeners')) return [];
    final sources = workspace ?? SourceWorkspace();
    final issues = <StaticIssue>[];
    for (final file in files) {
      if (shouldIgnoreStateFile(file, projectPath, config)) continue;
      ...
      for (final finding in visitor.findings) {
        issues.add(StaticIssue(
          ...
          framework: StateManagementFramework.provider,
          evidence: limitedEvidence(finding.notifications.map(compactEvidence)),
        ));
      }
    }
    return issues;
