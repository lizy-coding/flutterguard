...
class BleScanningRule {
  ...
  List<StaticIssue> _checkFile(
    ...
    for (final cls in unit.declarations.whereType<ClassDeclaration>()) {
      ...
      if (methodNames.contains('startScan') && !methodNames.contains('stopScan')) {
        final startScanMethod = methods.firstWhere((m) => m.name.lexeme == 'startScan');
      ...
      _checkScanTimeout(file, startScanMethod: methods, lineInfo: lineInfo, issues: issues);
    }
    ...
    for (final method in startScanMethod) {
      ...
      if (!hasTimeout) {
        ...
        issues.add(StaticIssue(
          ...
          suggestion: '为 startScan() 添加超时参数 (推荐 < ${config.maxScanDurationMs}ms)',
          metadata: {
